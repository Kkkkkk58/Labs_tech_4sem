...
public class BanksTest {
	...
	@BeforeEach
	public void setup() {
		FastForwardingSubscribableClock clock = new FastForwardingSubscribableClock(LocalDateTime.now(), ZoneId.systemDefault());
		this.clock = clock;
	...
	public void replenishMoney_BalanceIncreases()
	{
		...
		DebitAccountType accountType = bank.getAccountTypeManager().createDebitAccountType(BigDecimal.valueOf(12), Period.ofMonths(1));
		MoneyAmount initialBalance = new MoneyAmount(BigDecimal.valueOf(15));
		UnmodifiableBankAccount account = bank.createDebitAccount(accountType, client, initialBalance);
	...
	public void transferMoney_ReceiverGetsMoneySenderGivesMoney()
	{
		...
		DebitAccountType accountType = bank.getAccountTypeManager().createDebitAccountType(BigDecimal.valueOf(12), Period.ofMonths(1));
		MoneyAmount initialBalance = new MoneyAmount(BigDecimal.valueOf(15));
		UnmodifiableBankAccount sender = bank.createDebitAccount(accountType, client, initialBalance);
		UnmodifiableBankAccount receiver = bank.createDebitAccount(accountType, client, initialBalance);
	...
	public void performOperationBeyondSuspiciousOperationLimit_ThrowsException()
	{
		...
		Assertions.assertThrows(TransactionException.class, () -> centralBank.replenish(account.getId(), new MoneyAmount(BigDecimal.valueOf(15))));
	}
	...
	public void reachDebtWithNonNegativeBalanceAccount_ThrowsException()
	{
		...
		Assertions.assertThrows(TransactionException.class, () -> centralBank.withdraw(account.getId(), new MoneyAmount(BigDecimal.ONE)));
	}
	...
	public void reachDebtWithNegativeBalanceAvailableBankAccount_HasDebt()
	{
		...
		Customer client = getVerifiedCustomer();
		bank.registerCustomer(client);
		CreditAccountType accountType = bank.getAccountTypeManager().createCreditAccountType(new MoneyAmount(BigDecimal.valueOf(15)), new MoneyAmount(BigDecimal.ZERO));
		UnmodifiableBankAccount account = bank.createCreditAccount(accountType, client, new MoneyAmount(BigDecimal.ZERO));
	...
	public void reachDebtLimitWithCreditAccount_ThrowsException()
	{
		...
		Assertions.assertThrows(TransactionException.class, () -> centralBank.withdraw(account.getId(), new MoneyAmount(BigDecimal.valueOf(20))));
	}
	...
	public void creditAccountWithDebt_AccountCharged()
	{
		...
		CreditAccountType accountType = bank.getAccountTypeManager().createCreditAccountType(new MoneyAmount(BigDecimal.valueOf(10000)), charge);
		UnmodifiableBankAccount account = bank.createCreditAccount(accountType, client, new MoneyAmount(BigDecimal.ZERO));

		MoneyAmount withdrawal = new MoneyAmount(BigDecimal.ONE);
		centralBank.withdraw(account.getId(), withdrawal);
		centralBank.withdraw(account.getId(), withdrawal);
		Assertions.assertEquals((withdrawal.multipliedBy(BigDecimal.valueOf(2))).add(charge), account.getDebt());
	}
	...
	public void depositAccountWithdrawalBeforeLimit_ThrowsException()
	{
		...
		DepositAccountType accountType = bank.getAccountTypeManager().createDepositAccountType(depositTerm, interestOnBalancePolicy, Period.ofMonths(1));
		UnmodifiableBankAccount account = bank.createDepositAccount(accountType, client, new MoneyAmount(BigDecimal.ZERO));
	...
	public void depositAccountsWithDifferentBalances_AccountsHaveSuitableInterestOnBalance()
	{
		...
		Assertions.assertEquals(BigDecimal.valueOf(3), accountType.getInterestPercent(new MoneyAmount(BigDecimal.valueOf(49_999))));
		Assertions.assertEquals(BigDecimal.valueOf(3.5), accountType.getInterestPercent(new MoneyAmount(BigDecimal.valueOf(50_000))));
	}
	...
	public void accountWithInterestOnBalance_BalanceIncreasedAfterTimeSkip()
	{
		...
		DebitAccountType accountType = bank.getAccountTypeManager().createDebitAccountType(interestOnBalance, interestCalculationPeriod);
		MoneyAmount initialBalance = new MoneyAmount(BigDecimal.valueOf(15));
		UnmodifiableBankAccount account = bank.createDebitAccount(accountType, client, initialBalance);
	...
	public void cancelTransaction_AccountBalancesRemainAsBefore()
	{
		...
		MoneyAmount transfer = new MoneyAmount(BigDecimal.valueOf(12));
		ReadOnlyOperationInformation operationInformation = centralBank.transfer(sender.getId(), receiver.getId(), transfer);
		Assertions.assertEquals(initialBalance.subtract(transfer), sender.getBalance());
		Assertions.assertEquals(initialBalance.add(transfer), receiver.getBalance());
	...
	public void cancelCancelledTransaction_ThrowsException()
	{
		...
		centralBank.cancelTransaction(operationInformation.getId());
		Assertions.assertThrows(TransactionStateException.class, () -> centralBank.cancelTransaction(operationInformation.getId()));
	}
	...
	private InterestOnBalancePolicy getInterestOnBalancePolicy()
	{
		return new InterestOnBalancePolicy(List.of(
			new InterestOnBalanceLayer(new MoneyAmount(BigDecimal.ZERO), BigDecimal.valueOf(3)),
				new InterestOnBalanceLayer(new MoneyAmount(BigDecimal.valueOf(50_000)), BigDecimal.valueOf(3.5)),
				new InterestOnBalanceLayer(new MoneyAmount(BigDecimal.valueOf(100_000)), BigDecimal.valueOf(4))
		));
	}

	private NoTransactionalBank getBank(MoneyAmount suspiciousOperationsLimit)
	{
		return centralBank.registerBank(new BankImpl("Krugloff", accountFactory, suspiciousOperationsLimit, clock));
	}

	private MoneyAmount getExpectedBalanceAfterInterestCalculation(MoneyAmount initialBalance, BigDecimal interestOnBalance)
	{
		return initialBalance
			.multipliedBy(BigDecimal.ONE.add(interestOnBalance.divide(BigDecimal.valueOf(Year.of(LocalDateTime.now(clock).getYear()).length()), 10, RoundingMode.HALF_UP)));
	}
